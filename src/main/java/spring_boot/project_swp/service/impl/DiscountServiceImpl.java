...
public class DiscountServiceImpl implements DiscountService {
    ...
    public DiscountResponse getDiscountById(Integer discountId) {
        Discount discount = discountRepository.findById(discountId)
                .orElseThrow(() -> new NotFoundException("Không tìm thấy mã giảm giá với ID: " + discountId));
    ...
    @Override
    public DiscountResponse updateDiscount(Integer discountId, DiscountRequest request) {
        ...
        if (discountWithSameCode.isPresent() && !discountWithSameCode.get().getDiscountId().equals(discountId)) {
            throw new ConflictException("Mã giảm giá đã tồn tại");
        }
    ...
    public void deleteDiscount(Integer discountId) {
        if (!discountRepository.existsById(discountId)) {
            throw new NotFoundException("Không tìm thấy mã giảm giá với ID: " + discountId);
        }
    ...
    public DiscountResponse getDiscountByCode(String code) {
        Discount discount = discountRepository.findByCode(code)
                .orElseThrow(() -> new NotFoundException("Không tìm thấy mã giảm giá với Code: " + code));
        return discountMapper.toDiscountResponse(discount);
    }
}